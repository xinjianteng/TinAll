...
import com.cliff.reader.bean.BookSource;
...
import com.cliff.reader.bean.support.DownloadMessage;
import com.cliff.reader.bean.support.DownloadProgress;
import com.cliff.reader.bean.support.DownloadQueue;
import com.cliff.reader.bean.support.ReadTheme;
import com.cliff.reader.component.AppComponent;
import com.cliff.reader.component.DaggerBookComponent;
import com.cliff.reader.manager.CacheManager;
...
import com.cliff.reader.manager.setting.BookMarksManager;
import com.cliff.reader.manager.setting.BrightnessManager;
import com.cliff.reader.manager.setting.FontSizeManager;
import com.cliff.reader.manager.setting.ReadThemeManager;
import com.cliff.reader.service.DownloadBookService;
import com.cliff.reader.ui.adapter.BookMarkAdapter;
import com.cliff.reader.ui.adapter.TocListAdapter;
import com.cliff.reader.ui.contract.BookReadContract;
import com.cliff.reader.ui.easyadapter.ReadThemeAdapter;
import com.cliff.reader.ui.presenter.BookReadPresenter;
import com.cliff.reader.utils.FileUtils;
import com.cliff.reader.utils.FormatUtils;
...
import com.cliff.reader.view.readview.BaseReadView;
import com.cliff.reader.view.readview.OnReadStateChangeListener;
import com.cliff.reader.view.readview.OverlappedWidget;
import com.cliff.reader.view.readview.PageWidget;
...
public class ReadActivity extends BaseActivity implements BookReadContract.View {
    ...
    TextView mTvBookReadReading;//朗读

    @Bind(R.id.tvBookReadCommunity)
    TextView mTvBookReadCommunity;//社区
    ...
    private void initAASet() {
        curTheme = ReadThemeManager.getInstance().getReadTheme();
        ReadThemeManager.getInstance().setReaderTheme(curTheme, mRlBookReadRoot);

        seekbarFontSize.setMax(10);
        //int fontSizePx = SettingManager.getInstance().getReadFontSize(bookId);
        int fontSizePx = FontSizeManager.getInstance().getReadFontSize();
        ...
        seekbarLightness.setProgress(BrightnessManager.getInstance().getReadBrightness());
        isAutoLightness = ScreenUtils.isAutoBrightness(this);
        if (BrightnessManager.getInstance().isAutoBrightness()) {
            startAutoLightness();
        } else {
            stopAutoLightness();
        }
        ...
        cbAutoBrightness.setChecked(BrightnessManager.getInstance().isAutoBrightness());
        cbAutoBrightness.setOnCheckedChangeListener(new ChechBoxChangeListener());

        gvAdapter = new ReadThemeAdapter(this, (themes = ReadThemeManager.getInstance().getReaderThemeData(curTheme)), curTheme);
    ...
    public void readBook() {
        ...
        ToastUtils.showToast("朗读--正在拼命开发中...");
    }

    @OnClick(R.id.tvBookReadCommunity)
    public void onClickCommunity() {//社区
        gone(rlReadAaSet, rlReadMark);
        ToastUtils.showLongToast("社区");
...
//        BookSourceActivity.start(this, bookId, 1);
    ...
    private void changedMode(boolean isNight, int position) {
        ...
        mPageWidget.setTheme(isNight ? ReadThemeManager.NIGHT : curTheme);
    ...
    public void brightnessMinus() {
        int curBrightness = BrightnessManager.getInstance().getReadBrightness();
        if (curBrightness > 2 && !BrightnessManager.getInstance().isAutoBrightness()) {
            ...
            BrightnessManager.getInstance().saveReadBrightness(curBrightness);
        }
    }
    ...
    public void brightnessPlus() {
        int curBrightness = BrightnessManager.getInstance().getReadBrightness();
        if (curBrightness < 99 && !BrightnessManager.getInstance().isAutoBrightness()) {
            seekbarLightness.setProgress((curBrightness = curBrightness + 2));
            ScreenUtils.setScreenBrightness(curBrightness, ReadActivity.this);
            BrightnessManager.getInstance().saveReadBrightness(curBrightness);
        }
    ...
    public void clearBookMark() {
        BookMarksManager.getInstance().clearBookMarks(bookId);
    ...
    public void addBookMark() {
        ...
        if (BookMarksManager.getInstance().addBookMark(bookId, mark)) {
    ...
    private void updateMark() {
        ...
        mMarkList = BookMarksManager.getInstance().getBookMarks(bookId);
...
//            mTtsPlayer.stop();
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (seekBar.getId() == seekbarFontSize.getId() && fromUser) {
                calcFontSize(progress);
                    ...
                    && !BrightnessManager.getInstance().isAutoBrightness()) { // 非自动调节模式下 才可调整屏幕亮度
                ScreenUtils.setScreenBrightness(progress, ReadActivity.this);
                BrightnessManager.getInstance().saveReadBrightness(progress);
            }
        }
    ...
    private void startAutoLightness() {
        BrightnessManager.getInstance().saveAutoBrightness(true);
        ScreenUtils.startAutoBrightness(ReadActivity.this);
        seekbarLightness.setEnabled(false);
    }

    private void stopAutoLightness() {
        BrightnessManager.getInstance().saveAutoBrightness(false);
        ScreenUtils.stopAutoBrightness(ReadActivity.this);
        int value = BrightnessManager.getInstance().getReadBrightness();
