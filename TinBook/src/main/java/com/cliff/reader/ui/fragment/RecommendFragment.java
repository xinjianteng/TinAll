package com.cliff.reader.ui.fragment;
...
import com.cliff.reader.base.BaseRVFragment;
...
import com.cliff.reader.bean.support.RefreshCollectionListEvent;
import com.cliff.reader.bean.support.UserSexChooseFinishedEvent;
...
import com.cliff.reader.ui.activity.MainActivity;
import com.cliff.reader.ui.activity.ReadActivity;
import com.cliff.reader.ui.activity.ReadPDFActivity;
import com.cliff.reader.ui.activity.ScanLocalBookActivity;
import com.cliff.reader.ui.contract.RecommendContract;
import com.cliff.reader.ui.easyadapter.RecommendAdapter;
import com.cliff.reader.ui.presenter.RecommendPresenter;
import com.cliff.reader.view.recyclerview.adapter.RecyclerArrayAdapter;
...
//        ReadActivity.startActivity(activity, mAdapter.getItem(position), mAdapter.getItem(position).isFromSD);
        ...
        if (mAdapter.getItem(position).path.endsWith(Constant.SUFFIX_TXT)) {
            // TXT
            ReadActivity.startActivity(activity,  mAdapter.getItem(position),  mAdapter.getItem(position).isFromSD);
        } else if (mAdapter.getItem(position).path.endsWith(Constant.SUFFIX_PDF)) {
            // PDF
            ReadPDFActivity.start(activity,  mAdapter.getItem(position).path);
        } else if (mAdapter.getItem(position).path.endsWith(Constant.SUFFIX_EPUB)) {
            ReadPDFActivity.start(activity,  mAdapter.getItem(position).path);
            // EPub
//          ReadEPubActivity.start(ScanLocalBookActivity.this, books.path);
        } else if (mAdapter.getItem(position).path.endsWith(Constant.SUFFIX_CHM)) {
            // CHM
//           ReadCHMActivity.start(ScanLocalBookActivity.this, books.path);
        }
    }
