 ...
 */
package com.cliff.reader.ui.easyadapter;
...
import com.cliff.reader.manager.setting.ReadProgressManager;
import com.cliff.reader.utils.FileUtils;
import com.cliff.reader.utils.FormatUtils;
import com.cliff.reader.view.recyclerview.adapter.BaseViewHolder;
        ...
        return new BaseViewHolder<Recommend.RecommendBooks>(parent, R.layout.item_recommend_list) {
            ...
            public void setData(final Recommend.RecommendBooks item) {
                ...
                if (item.path != null && item.path.endsWith(Constant.SUFFIX_PDF)) {
                    holder.setImageResource(R.id.ivRecommendCover, R.drawable.ic_file_pdf);
                } else if (item.path != null && item.path.endsWith(Constant.SUFFIX_EPUB)) {
                    holder.setImageResource(R.id.ivRecommendCover, R.drawable.ic_file_epub);
                } else if (item.path != null && item.path.endsWith(Constant.SUFFIX_CHM)) {
                    holder.setImageResource(R.id.ivRecommendCover, R.drawable.ic_file_chm);
                } else if (item.isFromSD) {
                    holder.setImageResource(R.id.ivRecommendCover, R.drawable.ic_file_txt);
                    long fileLen = FileUtils.getChapterFile(item._id, 1).length();
                    if (fileLen > 10) {
                        double progress = ((double) ReadProgressManager.getInstance().getReadProgress(item._id)[2]) / fileLen;
