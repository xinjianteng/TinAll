...
public abstract class BaseReadView extends View {
    ...
    protected Bitmap mCurPageBitmap,//当前页面
                     mNextPageBitmap;//下一页
    protected Canvas mCurrentPageCanvas,//当前页面
                     mNextPageCanvas;//下一页
    ...
    public synchronized void init(int theme) {
        if (!isPrepared) {
            try {
                pagefactory.setBgBitmap(ReadThemeManager.getInstance().getThemeDrawable(theme));
                // 自动跳转到上次阅读位置
                int pos[] = ReadProgressManager.getInstance().getReadProgress(bookId);
    ...
    public synchronized void setFontSize(final int fontSizePx) {
        ...
        if (isPrepared) {
            ...
            FontSizeManager.getInstance().saveFontSize(fontSizePx);
            postInvalidate();
        }
    }
