 ...
 */
package com.cliff.reader.view.readview;
...
public class PageFactory {
     ...
     * 字体路径
     */
    private String[] fontPaths={ "font/kaiti.ttf",
                                 "font/songti.ttf"};

    private int fontPathSelect=0;

    public PageFactory(Context context, String bookId, List<BookMixAToc.mixToc.Chapters> chaptersList) {
        this(context,
                ScreenUtils.getScreenWidth(),
                ScreenUtils.getScreenHeight(),
                FontSizeManager.getInstance().getReadFontSize(),
                bookId,
                chaptersList);
    }
    ...
    public PageFactory(Context context,
        ...
        Typeface typeface = Typeface.createFromAsset(context.getAssets(),fontPaths[fontPathSelect]);
    ...
    public synchronized void onDraw(Canvas canvas) {
        ...
        if (mLines.size() > 0) {
            ...
            // 保存阅读进度
            ReadProgressManager.getInstance().saveReadProgress(bookId, currentChapter, curBeginPos, curEndPos);
        }
    }
    ...
    public void convertBetteryBitmap() {
        ...
        batteryView.setProgressDrawable(ContextCompat.getDrawable(mContext,
                ReadThemeManager.getInstance().getReadTheme() < 4 ?
