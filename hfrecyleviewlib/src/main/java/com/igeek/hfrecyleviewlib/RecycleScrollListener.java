...
public abstract class RecycleScrollListener extends RecyclerView.OnScrollListener {
    ...
    RelativeLayout downloadStateRl;//下载

    public void setmBookDetails_Rl_bg(RelativeLayout downloadStateRl) {
        this.downloadStateRl = downloadStateRl;
    }
    ...
    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        if(downloadStateRl!=null){
            boolean isSignificantDelta = Math.abs(dy) > 4;
            if (isSignificantDelta) {
                if (dy > 0) {
                    //上
                    downloadStateRl.setVisibility(View.GONE);
                } else {
                    //下
                    downloadStateRl.setVisibility(View.VISIBLE);
                }
            }
        }
