...
public class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private static final float STROKE_WIDTH_LARGE = 4f;
...
//            }
            ...
            drawTriangle(c, startAngle, sweepAngle, bounds, mPaint);
        }

        //三角
        private void drawTriangle(Canvas c, float startAngle, float sweepAngle, Rect bounds,Paint paint) {
...
//            Bitmap mBitmap = Bitmap.createBitmap(20, 20, Bitmap.Config.ARGB_8888);
            if (mShowArrow) {
                ...
                // Adjust the position of the triangle so that it is inset as 调整三角形的位置，使其插入
                // much as the arc, but also centered on the arc.   多为弧，又以弧为中心
                //TODO XZD  微调箭头位置
                float inset = (int) mStrokeInset / 2 * mArrowScale;
                float x = (float) (mRingCenterRadius * Math.cos(0) + bounds.exactCenterX());
                float y = (float) (mRingCenterRadius * Math.sin(0) + bounds.exactCenterY()) + paint.getStrokeWidth()/3;

                //TODO 0 RED
//                paint1.setColor(Color.RED);
//                c.drawPoint(x+paint.getStrokeWidth()/2, y, paint1);
                // Update the path each time. This works around an issue in SKIA   每次更新路径。
                // where concatenating a rotation matrix to a scale matrix  在连接一个旋转矩阵规模的矩阵
                // ignored a starting negative rotation. This appears to have  忽略启动负旋转
                // been fixed as of API 21.
                mArrow.moveTo(0 + paint.getStrokeWidth()/2, 0);
                //TODO 1 BLUE
//                paint1.setColor(Color.BLUE);
//                c.drawPoint(0, 0, paint1);
                mArrow.lineTo(mArrowWidth * mArrowScale + paint.getStrokeWidth()/2, 0);

                //TODO 2 YELLOW
//                paint1.setColor(Color.YELLOW);
//                c.drawPoint(mArrowWidth * mArrowScale, 0, paint1);
                mArrow.lineTo(((mArrowWidth * mArrowScale + paint.getStrokeWidth()) / 2), (mArrowHeight * mArrowScale));
                //TODO 3 GREEN
//                paint1.setColor(Color.GREEN);
//                c.drawPoint((mArrowWidth * mArrowScale / 2), (mArrowHeight * mArrowScale), paint1);
                mArrow.offset(x - inset, y);
                //TODO 4 GRAY
//                paint1.setColor(Color.GRAY);
//                c.drawPoint(x - inset, y, paint1);//x - inset中点位置
                mArrow.close();
                // draw a triangle
                mArrowPaint.setColor(mCurrentColor);
//                mArrowPaint.setAlpha(150);
