...
public class NestedRefreshLayout extends ViewGroup
    ...
    public NestedRefreshLayout(Context context, AttributeSet attrs) {
        ...
        isPull=ta.getBoolean(R.styleable.NestedRefreshLayout_isPull,true);
        ...
        if(isPull){
            ...
            addOrUpdatePullView(pullViewHeight, null);
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        try {
            ...
            if(isPull){
                ...
                pullView.layout(childLeft, offsetTop - bottomScrollY, childLeft + childWidth, offsetTop + pullView.getMeasuredHeight() - bottomScrollY);

            }
        }catch (Exception ex){
            Log.e("NestedRefreshLayout 异常","NestedRefreshLayout  ex=="+ex.getMessage());
        }
...
//        final boolean canFling = canFilngUp || canFilngDown;
    ...
    private final PullAnimationListener refreshingListener=new PullAnimationListener() {
        ...
        public void start(Animation animation) {
            if(null!=pullRefreshView){
                pullRefreshView.onPullRefresh();//开始刷新
        ...
        public void end(Animation animation) {
            if(null!=pullRefreshView){
                if(pullHelper.getScroll()==0){
                    ...
                    pullRefreshView.onPullHided();//刷新完成
                }
            }
        }
